...
from misc.utils import PrioritizedReplayBuffer, FactorizedNoisyDuelingQNetwork
...
class DDDQNNoisyAgentPER():
    def __init__(self, config: DDDQNNoisyConfig) -> None:
        self.config = config
        ...
        self.policy_net: FactorizedNoisyDuelingQNetwork= FactorizedNoisyDuelingQNetwork(self.state_dim, config.n_actions).to(self.device)
        self.target_net: FactorizedNoisyDuelingQNetwork = FactorizedNoisyDuelingQNetwork(self.state_dim, config.n_actions).to(self.device)
